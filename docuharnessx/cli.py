...
def _init_command(args: argparse.Namespace, *, input_fn: "Any" = None) -> int:
    ...
    * otherwise (non-interactive, no ``--default``) → there is nothing to build, so
    ...
    compliance = load_compliance(project_dir)
    if not args.default:
        ...
        compliance = prompt_compliance_topics(
            input_fn=reader, out=sys.stdout, existing=compliance
        )
    ...
    if args.default:
        ...
        from docuharnessx.comprehension.compliance import ComplianceSelection

        compliance = ComplianceSelection()
    site_path = save_site_presentation(args.project_dir, presentation)
    compliance_path = save_compliance(args.project_dir, compliance)
    ...
    print(f"dhx init: wrote compliance selection: {compliance_path}")
