...
related: []
---
# What does [analysis](glossary.md#analysis) do?
...
`docuharnessx.analysis` is the **pure, model-free repository scanning + [analysis](glossary.md#analysis) core** of [DocuHarnessX](glossary.md#docuharnessx). Its own package docstring describes it as "the deterministic, side-effect-free core that turns a target repository on local disk into a frozen `RepoAnalysis`" (`docuharnessx/analysis/__init__.py:1`), and stresses that it is stdlib-only, harness-free, and unit-testable without any model — only the stage adapters in `docuharnessx/stages/ingest.py` and `docuharnessx/stages/analyze.py` know about HarnessX (`docuharnessx/analysis/__init__.py:5-7`).

## The [pipeline](glossary.md#pipeline): [scan](glossary.md#scan) → [aggregate](glossary.md#aggregate) → freeze

The core is a three-part [composition](glossary.md#composition) whose single entry point is `analyze(inv: FileInventory) -> RepoAnalysis` (`docuharnessx/analysis/analyzer.py:84`).
...
In short: [analysis](glossary.md#analysis) turns a local repository into one frozen, versioned, fully pre-sorted `RepoAnalysis` value object — languages/LOC, structure, entrypoints, build files, CI, tests, dependencies, components, public surface, docs, artifacts, and [scan](glossary.md#scan) stats — with no model calls and no network access, and an optional LLM layer that is strictly additive to that core.
