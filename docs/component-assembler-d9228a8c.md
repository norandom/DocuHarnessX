---
id: [component]([glossary](glossary.md#glossary).md#component):[assembler]([glossary](glossary.md#glossary).md#assembler)
title: What does [assembler]([glossary](glossary.md#glossary).md#assembler) do?
subjects:
- [assembler]([glossary](glossary.md#glossary).md#assembler)
summary: '`docuharnessx.assembler` is [DocuHarnessX]([glossary](glossary.md#glossary).md#docuharnessx)''s **MkDocs site-assembly core**
  ...
  **Material for MkDocs** source tree. Its own docstring names it the deterministic,
  ...
  [ontology](glossary.md#ontology) `Segment` set from the frozen `ReviewReport`, the loaded project `Vocabulary`,
  and an optional `RepoAnalysis`, then "emits a Material for MkDocs source tree under
  the [run](glossary.md#run)''s output directory: one `docs/*.md` [page](glossary.md#page) per accepted [segment](glossary.md#segment), per-role
  landing [pages](glossary.md#pages) with COBESY-structured intent-ordered agendas, a tags index, and a
...
related: []
---
# What does [assembler]([glossary](glossary.md#glossary).md#assembler) do?

<div class="dhx-layer" data-min="1" markdown="1">

`docuharnessx.assembler` is [DocuHarnessX]([glossary](glossary.md#glossary).md#docuharnessx)'s **MkDocs site-assembly core** — the "pure, model-free" stage that turns quality-gated content into a publishable **Material for MkDocs** source tree. Its own docstring names it the deterministic, harness-free core behind the thin `AssembleStage` adapter: it consumes the accepted [ontology](glossary.md#ontology) `Segment` set from the frozen `ReviewReport`, the loaded project `Vocabulary`, and an optional `RepoAnalysis`, then "emits a Material for MkDocs source tree under the [run](glossary.md#run)'s output directory: one `docs/*.md` [page](glossary.md#page) per accepted [segment](glossary.md#segment), per-role landing [pages](glossary.md#pages) with COBESY-structured intent-ordered agendas, a tags index, and a `mkdocs.yml`" (`docuharnessx/assembler/__init__.py:1-13`). Everything downstream imports from this one public namespace rather than the submodules (`docuharnessx/assembler/__init__.py:15-18`).
...
2. renders one `docs/<segment>.md` per accepted [segment](glossary.md#segment) via `render_segment_page`, passing the accepted-id set so cross-links resolve only to accepted [pages](glossary.md#pages) (`docuharnessx/assembler/writer.py:190-195`);
3. renders `docs/<role>/index.md` for each [vocabulary]([glossary](glossary.md#glossary).md#vocabulary) role that has at least one accepted [segment](glossary.md#segment), in [vocabulary]([glossary](glossary.md#glossary).md#vocabulary) order (`_emitted_roles`, `docuharnessx/assembler/writer.py:200-207`);
...
6. builds `mkdocs.yml` via `build_mkdocs_yaml` and returns a frozen `AssembledSite` carrying the site paths, identity, and [page](glossary.md#page) counts (`docuharnessx/assembler/writer.py:238-250`).
...
## 3. Render each [artifact]([glossary](glossary.md#glossary).md#artifact) deterministically

The renderers are pure, model-free, byte-stable transforms:

- **[Segment](glossary.md#segment) [pages](glossary.md#pages)** — `pages.py`: `page_filename` maps a [segment](glossary.md#segment) id to `<slug>-<sha256-digest-8>.md` so distinct ids never collide even when slugs do (`docuharnessx/assembler/pages.py:75-88`); `render_segment_page` emits YAML frontmatter whose `tags:` is exactly `emit_tags(segment, vocab)`, the title as H1, the [segment](glossary.md#segment) body verbatim, and a "Related" section filtered to accepted ids (`docuharnessx/assembler/pages.py:91-120`).
- **Role landing [pages](glossary.md#pages)** — `roles.py`: `render_role_landing_page` builds a COBESY SCQA opener (Situation/Complication/[Question](glossary.md#question)/Answer) framed only from the role's [vocabulary]([glossary](glossary.md#glossary).md#vocabulary) `label`/`description`, an intent-ordered guided agenda of links derived through `build_role_view` (no body duplication), and a Material `!!! info` role-switch admonition listing the other role [pages](glossary.md#pages) (`docuharnessx/assembler/roles.py:142-243`). `role_page_path` owns the `<role>/index.md` path rule with a filesystem-safe slug from `_slug` (`docuharnessx/assembler/roles.py:63-84`).
- **Home [page](glossary.md#page)** — `home.py`: `render_home_page(identity, role_pages)` produces the docs-root `index.md`: a heading naming the *target* project, a "choose your path" index over role [pages](glossary.md#pages), and a pointer to the tags index (`docuharnessx/assembler/home.py:33-74`); `render_question_home` is the question-list variant that also embeds `render_home_diagrams` (`docuharnessx/assembler/home.py:77-102`).
- **Mermaid companions** — `graphs.py`: `render_page_diagrams` and `render_home_diagrams` derive small flowcharts from the [page](glossary.md#page) record (related [pages](glossary.md#pages), cited files, [analysis]([glossary](glossary.md#glossary).md#analysis) entrypoints/components/public surface/build/[tests](glossary.md#tests)) and emit them above question-page prose via a tiny deterministic `_Graph` builder (`docuharnessx/assembler/graphs.py:274-317`).
...
Separately, `assemble_question_site(pages, identity, out_dir, analysis)` is the explore-first, question-organised entry point: it writes a Material tree from accepted [question](glossary.md#question) `Page` values only — home lists [question](glossary.md#question) titles, nav is home + [pages](glossary.md#pages), no per-role landings — and returns `None` (writing nothing under `site/`) when no [pages](glossary.md#pages) are accepted so callers skip deploy (`docuharnessx/assembler/question_site.py:48-99`).

In short: [assembler]([glossary](glossary.md#glossary).md#assembler) is the deterministic "renderer + writer" core that takes the frozen [review](glossary.md#review) output and [vocabulary]([glossary](glossary.md#glossary).md#vocabulary), resolves the target project's site identity from its git remote, renders per-segment [pages](glossary.md#pages), per-role COBESY landing [pages](glossary.md#pages), a home [page](glossary.md#page), tags index, extra CSS, and Mermaid diagrams, generates a byte-stable `mkdocs.yml`, writes the entire Material-for-MkDocs tree under `<out_dir>/site`, and returns an immutable `AssembledSite` seam for the deploy stage — with an alternate question-organised writer (`assemble_question_site`) as the explore-first path.
