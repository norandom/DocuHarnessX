---
id: [component]([glossary](glossary.md#glossary).md#component):[docuharnessx]([glossary](glossary.md#glossary).md#docuharnessx)
title: What does [docuharnessx]([glossary](glossary.md#glossary).md#docuharnessx) do?
subjects:
- [docuharnessx]([glossary](glossary.md#glossary).md#docuharnessx)
summary: [DocuHarnessX]([glossary](glossary.md#glossary).md#docuharnessx) is a Python package — version `2.0.0` (`docuharnessx/__init__.py:5`)
...
related: []
---
# What does [docuharnessx]([glossary](glossary.md#glossary).md#docuharnessx) do?

<div class="dhx-layer" data-min="1" markdown="1">

[DocuHarnessX]([glossary](glossary.md#glossary).md#docuharnessx) is a Python package — version `2.0.0` (`docuharnessx/__init__.py:5`) — whose own docstring describes it as a tool to "generate grounded developer documentation from a software repository" (`docuharnessx/__init__.py:1`). In practice it is a CLI-driven [pipeline](glossary.md#pipeline) that scans a target repo, decides which software questions deserve documentation, runs bounded model agents to write those [pages](glossary.md#pages), and assembles the accepted ones into an MkDocs site. Its pyproject entry point is the console script `dhx = "docuharnessx.cli:main"` (`pyproject.toml:33`).
...
4. `write_questions(...)` in `docuharnessx.composition.explore_writer` runs **one bounded writer harness per [question](glossary.md#question)**; a missing model omits every [question](glossary.md#question) with reason `no_model` rather than substituting an outline (`composition/explore_writer.py:54-58`). Accepted [pages](glossary.md#pages) go into the project's **living [page](glossary.md#page) store** via `store.put(page)` (`pipeline/run.py:124-125`).
5. `assemble_question_site` writes a Material-for-MkDocs source tree under `<out>/site` **only when at least one [page](glossary.md#page) was accepted** (`pipeline/run.py:59-84`); otherwise the [run](glossary.md#run) is an "honest-empty" zero-page report that still exits 0.
