...
```bash
uvx --python 3.12 --from git+https://github.com/norandom/DocuHarnessX.git@v3.3.0 dhx --help
```

Install onto `PATH`:

```bash
uv tool install --python 3.12 git+https://github.com/norandom/DocuHarnessX.git@v3.3.0
dhx --help
```
...
The reusable workflow is `norandom/DocuHarnessX/.github/workflows/adopt.yml@v3.3.0`. This repository dogfoods it via `.github/workflows/dhx.yml` (same-commit checkout). On each source push it runs incremental `dhx ci` and commits MkDocs / living pages with a `[dhx]` message. GitHub Pages still builds from `docs.yml`.
